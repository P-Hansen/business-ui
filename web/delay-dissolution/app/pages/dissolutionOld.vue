...
import DatePicker from '~/components/Connect/DatePicker.vue'
...
// 5. Court order
const courtFileNumber = ref('')
const courtOrderDate = ref('')
const courtOrderDetails = ref('')
...
const formattedDate = computed(() => {
  //when input box is used for date
  if (selected.value === 'custom') {
    if (!customDate.value) return ''

    const parsed = new Date(customDate.value + 'T00:00:00') 
    const today = new Date()
    const years = parsed.getFullYear() - today.getFullYear()
    const months = parsed.getMonth() - today.getMonth()
    customMonths.value =  years * 12 + months
  ...
  //when radio buttons are used for date
...
const maxDelayDate = new Date()
maxDelayDate.setFullYear(maxDelayDate.getFullYear() + 2)
...
const submitDelay = () => {
  const payload = {
    ...
    // 1.Delay
    delayUntil: formattedDate.value, //radio date
    ...
    // 5.Folio/Reference Number
    folioNumber: folioNumber.value,

    // 6.Court Order
    courtOrder: {
      fileNumber: courtFileNumber.value,
      orderDate: courtOrderDate.value,
      details: courtOrderDetails.value
    }
  }
  ...
  buttonControl: {
    leftButtons: [
      ...
      },
      {
        ...
        variant: 'outline',
        onClick: () => alert('save and resume later clicked!')
      }
    ],
    rightButtons: [
      {
        ...
        <div class="py-10 space-y-10">
          ...
          <div class="card">
            <div class="form-row">
              ...
              <div class="form-controls">
                ...
                <br>
                <div class="input-box">
                  <UInput v-model='clientEmail' placeholder='Client Email Address (Optional)' />
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="card">
            <div class="form-row">
              ...
              <div class="form-controls">
                <div class="input-box">
                  <UInput v-model="legalName" placeholder="Your Name" />
                </div>
                <div class="check-box">
                  <UCheckbox v-model="certified" />
                  <p class="check-label">
          ...
          <div class="card">
            <div class="form-row">
              ...
              <div class="form-controls">
                <div class="input-box">
                  <UInput v-model="folioNumber" placeholder="Folio or Reference Number (Optional)" />
                </div>
              </div>
            </div>
          </div>
